...
- **Action** — ProcessGuidance: Check for ARGUMENTS and extract guidance.
  - **If** ARGUMENTS exists → acknowledge guidance; extract specific focus areas; confirm understanding with user
  - Proceed with comprehensive review while honoring context
  - User confirmation: "I see you've provided additional guidance: '[repeat their guidance]'. I understand this means I should [explain how you'll apply it during review]. Shall I proceed with this focus?"
...
- **Action** — GenerateReviewPrompt: Create comprehensive independent review agent prompt replacing bracketed placeholders:
...
- **Action** — DispatchAgent: Deploy independent code review agent with comprehensive prompt above.
...
- [ ] Review scope clearly defined with work boundaries established
- [ ] Work origin scenario identified (A: Task List, B: Plan, C: Ad-hoc)
- [ ] Completed work explicitly specified (tasks/plan/independent with specifics)
- [ ] Files modified/created explicitly listed with change types and purposes
- [ ] Related files identified for comprehensive review scope
- [ ] Work-specific context extracted from appropriate documentation
- [ ] Out-of-scope work and files explicitly identified and excluded
- [ ] Comprehensive review agent deployed with full analysis scope
- [ ] All severity levels and analysis categories included in review prompt
- [ ] Work & file scope boundaries validated in agent prompt
- [ ] Review findings received and categorized by severity
- [ ] Action plan created with prioritization (CRITICAL → HIGH → MEDIUM → LOW)
- [ ] User presented with numbered severity-based summary
- [ ] Review report saved without overwriting existing files (scoped filename used if default exists)
- [ ] Dashboard updated with review outcomes
- [ ] Single Next Steps footer rendered with clear recommendation
- [ ] Next steps guide read and relevant options sourced for footer
**Output Location** — DetermineOutputDir: Decide where to save the review report.
